...
---追踪变量
--追踪的变量可以是local,upvalue,global
--跟踪的层中,行变化时,如果追踪变量有变化则会提示
--@param[type=string] var 变量名列表，以空格隔开
--@param[type=integer] level 默认为0,表示只跟踪调用trace.trace的函数,>0表示会深入跟踪level层
function trace.trace(var , level)
	...
	trace.on = true
	info.last = {}
	-- 返回到调用trace.trace函数时,回调会设置成hook
